...
**2026-09-03:** Brandon confirmed no clones of the scoot repo on his laptop/home/work machines (CLAUDE.md's 3-machine note is historical).

**2026-09-08 correction — there ARE two checkouts, not one.** dreamlab (`/home/brandon/scoot`) is the **authoritative** checkout. A second checkout exists at `/home/steve/scoot` on the Steve host, bind-mounted into a container there, **319 commits / 103 days behind** (`c02f799`, 2026-05-28, vs `origin/main` 2026-09-07) and with **no Twilio or Anthropic credentials** (it cannot text a member; no double-bot risk). Source: `HANDOFF_BIGMO_RIM_v0.2.md` §3, written on Steve — UNVERIFIED from dreamlab (no network path between the hosts). Consequences: history rewrites/force-pushes here DO need a note to Steve; any development there must start from a pull. The earlier "only checkout" claim was the map disagreeing with the territory — see [[project_bigmo_rim_questionnaire]] / RIM_architecture §3.2.
