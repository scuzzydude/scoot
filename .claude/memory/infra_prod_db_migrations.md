...
**Data hygiene note (found 2026-07-28, cleaned 2026-09-08):** prod `scoots` table had two extra rows — `id 78`/`80`, slug `selfstake-<timestamp>-2`, name `T2` — that look like test fixtures leaked from a self-stake integration test run rather than real Scoots. Brandon decided 2026-09-08: delete. Deleted (RIM Phase 0.6) after a verified `pg_dump` restore test (`~/backups/scoot-2026-09-08.dump`, row counts identical across 32 tables); rows 78/80 and their two `scoot_members` rows (rocketman, flags 1028) removed; every other FK to `scoots` checked, zero dependents. Root cause (a self-stake integration test writing to prod) is NOT fixed — tests still point at the prod DB.
