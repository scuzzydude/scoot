  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="rounded-lg border border-white/10 bg-white/[0.02] flex items-center justify-center overflow-hidden min-h-[240px]">
          {isImage ? (
            <img src={url} alt={attachment.filename} className="max-w-full max-h-[60vh] object-contain" />
          ) : isPdf ? (
            <embed src={url} type="application/pdf" className="w-full h-[60vh]" />
          ) : (
