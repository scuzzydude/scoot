...
scoot_members.card_serial for known name matches.
...
def main():
    ...
    for row in rows:
        ...
        if username:
            link_lines.append(
                f"UPDATE scoot_members SET card_serial = {sql_escape(serial)} "
                f"WHERE scoot_id = {SCOOT_ID} AND user_id = "
                f"(SELECT id FROM users WHERE username = {sql_escape(username)});"
            )
        else:
    ...
    with open(out_path, "w") as f:
        ...
        f.write("\n".join(insert_lines))
        f.write("\n\n-- auto-linked scoot_members.card_serial (name match)\n\n")
        f.write("\n".join(link_lines))
        f.write("\n")
