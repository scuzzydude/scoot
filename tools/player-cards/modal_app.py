...
MODEL_DOWNLOADS = [
    # Tier 1 test (2026-08-19, PLAN_facial_likeness.md): tried the "Zero"
    # checkpoint (cagliostrolab/animagine-xl-4.0-zero, the pretraining-
    # stage release cagliostrolab positions as the base for LoRA/adapter
    # work) in place of the main animagine-xl-4.0 release, to test
    # FACIAL_LIKENESS_RESEARCH.md's hypothesis that identity adapters
    # fight the main release because it's diverged too far from the base
    # SDXL distribution they were calibrated against.
    #
    # RESULT: negative, and not just for identity -- with the identical
    # ControlNet pose/lineart input, seed, and PuLID settings as
    # 34-TEST-BRANDON-v6, Zero generated a figure facing AWAY from camera
    # (rear view, no face at all), where the main checkpoint reliably
    # produced a face-forward pose. Zero is a raw, less-refined
    # pretraining checkpoint -- it doesn't follow ControlNet conditioning
    # as precisely as the more-finetuned main release, which matters more
    # for this pipeline than whatever adapter-compatibility it might gain.
    # Reverted to the main release. See PLAN_facial_likeness.md Tier 2
    # (per-subject LoRA) for the next real step -- pending Brandon's call.
