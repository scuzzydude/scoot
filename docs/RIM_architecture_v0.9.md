_Version: v0.9 (rev 9) | 2026-09-16 | Status: DRAFT — **cleared for external distribution**. Supersedes v0.8 (`29209ab16`); v0.1-v0.8 stand as shipped. **v0.9 exists because the second machine asked the first one to answer its own questionnaire.** §3.8 had scored Steve 8/8 and BigMo 4/8, and three of v0.8's narrowings had come from BigMo pushing back. Measured on Steve, the score is 3 earned, 1 vacuous, 2 partial, 2 fail — so §3.8's scoring is withdrawn, §3.2 splits, §3.4 is suspended, and §3.0's warning acquires a worked example: its own author, one revision later. Working file: `Steve_RIM_QA_v0.1.answer.md`. Open items in §10._
...
**§3 was rescored against Steve itself, 2026-09-16**, at BigMo's request. It cost Steve five
of its eight claimed criteria and moved BigMo ahead on earned ones. §3.0, §3.2, §3.4 and §3.8.
...
### 3.0.1 The trap, caught in this document, one revision later

v0.8 wrote the warning above and then walked into it. Its §3.8 scored **Steve 8 of 8** — and
that score was never measured. Asked by the second machine to answer the questionnaire the
way it had, Steve earns **three**:

| Criterion | v0.8 claimed | Measured | How the claim had been established |
|---|---|---|---|
| 3.1 one artifact | meets | **fail** | asserted |
| 3.2 positional authority | meets | **partial** | constructed — the map exists; it cannot deny |
| 3.2b halt authority | meets | **fail** | asserted, from one sentence in a planning doc |
| 3.3 append-only | meets | **partial** | constructed — the log is; the fold is not |
| 3.4 non-blocking | meets | **vacuous** | measured, *and the criterion is that measurement* |
| 3.5 / 3.6 / 3.7 | meets | **meets** | earned |

**Two of the eight were the author's own assertions, and one was a description of Steve
wearing a disqualifier.** The full working is in `Steve_RIM_QA_v0.1.answer.md`; the
consequences are §3.2, §3.4 and §3.8.

**Why this is worth keeping in the document rather than quietly fixing.** §3.0 already said a
criterion derived from one machine is a description of that machine. It took an outside
challenge to apply that to the machine holding the pen — and the corpus has now recorded the
same shape five times (§7.7, §7.9, §5, §8.1, here): *a property asserted from the artifact
nearest to hand instead of from the thing itself.* Nearest to hand includes one's own work.
...
### 3.2 Write authority — positional **and enforced** [split in v0.9]

Two requirements, not one. v0.8 had only the first, and that is why it could not tell Steve's
map apart from an authority.

**(a) Positional basis.** Write authority derives from position in a tree. Not identity, not
rank, not seniority, not who asked first.

> **Disqualified if** write authority is by name. Names change across restarts; directories do
> not. A system whose owner-of-record is a person or a session name drifts silently.

**(b) Enforced in effect.** The authority can *deny*. A map that records who should write, and
permits everyone, is disclosure — useful, and not authority.

> **Disqualified if** the mechanism has no denial path at all.

**Why (b) had to be added, and it cost Steve this criterion.** Steve's map is positional, so it
passed v0.8's only disqualifier. But its hook says, in its own docstring:
...
Measured on the log: **9 `gate_warn` events, 9 writes permitted, and no event type for a denied
write exists.** Three on a contended file, five on another session's subtree, one on a `strict`
node. Steve therefore meets (a) and fails (b).

**The distinction was invisible from Steve** — advisory was the only kind of authority this host
had, so "advisory" never presented itself as disqualifying. It took a machine with an *enforced*
authority (§3.2b) to make the gap visible, which is the second time the second machine has
supplied a distinction the first could not see.

**Fail-open is a defensible choice.** It is not a defect to warn rather than block; a gate that
fails closed on a false positive is worse than one that discloses. But a system that chose
disclosure has chosen not to have write authority, and should score accordingly rather than take
credit for the map's existence.
...
### 3.4 Non-blocking convergence [**SUSPENDED** in v0.9 — untestable as written]

Participants never wait on *authoring*. Contended writes are handed to the resource's owner as a
payload and applied serially. Parallel-safe work proceeds in your own subtree.

**Disqualified if** authoring is serialized by a lock — not if anything is. A lock on an external
or physically serial resource (a carrier send rate, one GPU, a `flock` so a second worker exits
rather than queues) is correct and not a violation. v0.2 conflated the two; that narrowing stands.

> **⚠️ Suspended: no instance can currently fail this.** The criterion was written by generalising
> Steve's own best measurement — four sessions, eight artifacts, one contended file, zero idle
> time (§7.2). **A criterion derived from a machine's best day cannot fail that machine.** And it
> cannot test the second machine either, where nothing waits because there is only one author.
> So it currently passes both instances for opposite and equally uninformative reasons.

**What would revive it:** a measured wait. "No participant waits on authoring" is unfalsifiable
without instrumentation for how long a session actually waited on another's write, and neither
host has it. Until then this is a design principle the corpus believes and cannot check, which is
a different thing from a criterion — and §10 records it as such.

**The §7.2 measurement is not withdrawn.** Four sessions, eight artifacts, zero idle time
happened, and it is still the reason to believe the principle. It is evidence *for the design*,
not a test *of* it.
...
### 3.8 The two machines, scored — **withdrawn and re-measured**

v0.8's table is withdrawn. It scored Steve 8/8 without measuring, and two of those cells were
the author's own assertions. Replaced with measured verdicts and, for each, **how the claim was
established** — because that column is what the previous version was missing.

| # | Criterion | Steve | how established | BigMo | how established |
|---|---|---|---|---|---|
| 3.1 | One artifact | **fail** — 10 representations, drift documented | measured | **fail** — ≥5 representations | measured |
| 3.2 *basis* | Write authority is positional | meets — `ownership.toml` | constructed | fail, deliberately — identity | measured |
| 3.2 *enforcement* | Write authority can **deny** | **fail** — 9 warns, 9 allowed, no deny path | **measured** | n/a for writes | — |
| 3.2b | Halt authority by identity | **fail** — a planning-doc sentence, no hook | asserted | **meets** — hard-coded, non-delegable | measured |
| 3.3 | Append-only history | **partial** — log appends; fold is last-write-wins per item | measured | **partial** — snapshot with no log behind it | measured |
| 3.4 | Non-blocking authoring | **suspended** | untestable | **suspended** | untestable |
| 3.5 | Rank commits / fact adjudicates | meets | earned | **meets, named the second half** | earned |
| 3.6 | Design out, vocabulary in | meets | earned | **meets, forced the narrowing** | earned |
| 3.7 | Recall obligatory | meets — mechanised | earned | **meets, named it** | earned |

**Earned totals: BigMo 4, Steve 3.**

**The second machine scores higher than the first, and is the only one with an enforced
authority of any kind.** The 8-versus-4 gap in v0.8 was almost entirely an artifact of who held
the pen — which is the claim §3.0 makes about criteria in general, now demonstrated on its own
author rather than argued.

**What each machine actually has that the other does not**, which is more useful than a total:

| | Steve | BigMo |
|---|---|---|
| An append-only log at all | **yes** | no |
| Positional write authority | yes, advisory | no |
| An authority that can deny | no | **yes, for halts** |
| Mechanised recall | **yes** | by convention |
| Enough participants to have found its own drift | **yes** — and it did | no |

That last row is the one worth sitting with. **Steve fails 3.1 more clearly than BigMo does
precisely because it has enough participants to notice** — a memory asserting one repo checkout
when there were two, a memory naming the wrong artifact, a shared stylesheet deleted with four
pages rendering unstyled while every status check passed. BigMo's representations disagree too;
nothing there has had cause to check. **A machine that can see its own drift is not thereby
worse than one that cannot, and a scoring table that says otherwise is measuring visibility, not
health.** §10 records that as an open weakness in the criteria rather than resolving it here.

**Withdrawn along with the table:** v0.2's "a machine meeting 3.1 and 3.2 is doing the
load-bearing work." Neither machine meets 3.1, both are useful, so the claim was false as well
as Steve-shaped.
...
| **new** | **Instrument 3.4, or drop it.** Suspended in v0.9: no instance can fail it — Steve because the criterion is its own measurement, BigMo because n=1. Reviving it needs a measured wait time on authoring, which neither host records. | Blocking for any future scoring. A criterion nothing can fail is not a criterion, and the corpus now has one. |
| **new** | **Does the scoring reward blindness?** §3.8: Steve fails 3.1 *more clearly* than BigMo because it has enough participants to have found its own drift. As written, the criteria score a machine that cannot see its representations disagreeing above one that can. | Unresolved and it matters — it is the difference between measuring health and measuring visibility. Possibly wants a criterion about *detecting* drift rather than avoiding it. |
...
| Criterion | State |
|---|---|
| A reader who has never seen Steve or the chip-design instance can say what RIM is, and correctly place a new system | §1–§4. **Demonstrated twice now.** §4.2 was the framework catching a level error in the author's own framing. §3.0.1 is it catching a *scoring* error in its own previous revision, at the second machine's prompting — which is the stronger of the two, because nobody outside had to be wrong for it to work. |
...
**What v0.9 gave up.** Three of the eight criteria no longer produce a verdict for Steve, one
produces none for either machine, and the earned totals are 4–3 to the *second* machine. That is
a document scoring itself down on measurement, and it is the first revision where the pattern was
tested against the machine that wrote it rather than only against the one that did not.

**The claim that survives all of it** is §8's: these mechanisms are what a system grows under
concurrency pressure. Nothing in the rescore touches that — Steve still has the log, the map and
the mechanised recall that BigMo lacks, and still grew each of them after a specific recorded
collision. What the rescore removes is the implication that having grown them means having
finished.
